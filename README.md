...
1. **No committed sample transcript yet.** I built this without an API key
   available in the environment, so I could not record a real session, and I
   would rather ship an empty `transcripts/` directory than fabricate a
   transcript and present model output I never saw. Reproduction commands are in
   *Sample transcript* below; recording one is a two-minute job with a key.
   **This is the one deliverable gap.**
2. **No size-chart or vanity-sizing data.** Brand tips are the model's priors,
   not measurements from a garment database. Do not buy clothes based on this.
3. **The confidence weights are hand-tuned and uncalibrated.** 0.35 / 0.30 / 0.20
...
4. **`specificity` is a model judgement.** Python owns the arithmetic, but the
...
5. **Mixed metric/imperial input can be misparsed.** `"I'm 34"` is flagged
   ambiguous, but a confident-sounding `"90cm"` in a US context is not caught.
6. **Recommendation quality is unevaluated.** There is no eval set. The tests
   verify the scoring and state machine, not whether the advice is good.
7. **Two model calls per turn** (extraction, then response) roughly doubles
   latency and cost versus a single combined call. That is a deliberate trade —
   see *System prompt design* — but it is a real cost.
8. **English only**, and the four questions are fixed in order and number.
...
Default: **`claude-opus-5`**, overridable with `STELLA_MODEL` in `.env`.
...
score rather than just producing an awkward sentence. Opus 5 supports
`client.messages.parse()` with a Pydantic schema, so the JSON contract is
enforced server-side and validated client-side; I never parse free text or
regex model output.
...
**Cost and latency.** Two calls per turn, roughly 1.5-2.5K input tokens each
(the state block is deliberately terse), and five turns per session — order of
20 calls and well under a cent of input at Opus 5's $5/MTok. At that volume the
cost difference against `claude-sonnet-5` ($2/MTok) is not worth the calibration
risk. `STELLA_MODEL=claude-sonnet-5` is the documented downgrade for anyone
running many sessions; the code is model-agnostic.

I did not tune `output_config.effort`, which is the obvious next lever: the
extraction call is mechanical enough to likely hold up at `low`, which would cut
most of the per-turn latency. Untested, so not shipped.
