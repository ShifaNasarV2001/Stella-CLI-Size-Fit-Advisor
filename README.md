...
- **Full four-question interview**, model-driven end to end — opening turn
  included. There are no scripted conversational responses anywhere in the
  codebase (see *Verifying the no-scripted-responses constraint* below).
...
## Verifying the no-scripted-responses constraint

Every conversational sentence is generated by the model. The only hardcoded
strings are UI chrome: the progress bar, the slash-command help, error banners,
and the field labels on the recommendation card (`SIZE RANGE`, `SILHOUETTE`,
…) — the values beside those labels are all model-generated. To check:

```bash
grep -rn "print(" stella/ | grep -v "cli.py"     # no output: only the CLI prints
```

The judgement call worth naming: the recommendation card's **labels** are
hardcoded. I treated them as the same class of thing as the progress bar —
layout chrome around model-authored content — rather than as scripted dialogue.
The alternative was a third model call to narrate the structured object
conversationally, which costs latency to restate data the user can already read.

---

## Sample transcript

**Not yet recorded** — see limitation 1. To produce one:
